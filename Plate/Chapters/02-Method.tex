...
The systematic search and selection process followed \gls{prisma} guidelines to ensure transparent and reproducible methodology. \autoref{fig:prisma} illustrates the flow of studies through each phase of the systematic review.

\begin{landscapemode}{297mm}{210mm}
\begin{figure}[p]
    \centering
    \caption{PRISMA flow diagram showing systematic search and study selection process}
    \label{fig:prisma}
    \begin{tikzpicture}[node distance=2cm, auto,
        ...
        \node[box, below of=identification] (afterdup) {\textbf{After Duplicates Removed}\\Records after duplicates removed\\(n = 184)\\
        \small 103 duplicates identified and removed};
        
        % Screening phase
        \node[box, below of=afterdup] (screened) {\textbf{Screening}\\Records screened by title and abstract\\(n = 184)};
        
        % Excluded at screening
        \node[smallbox, right of=screened, xshift=6cm] (excluded1) {Records excluded\\(n = 108)\\
        \small Non-Australasian (n=67)\\Wrong population (n=23)\\No relevant outcomes (n=18)};
        
        % Eligibility assessment
        \node[box, below of=screened] (fulltext) {\textbf{Eligibility}\\Full-text articles assessed for eligibility\\(n = 76)};
        
        % Excluded at full-text
        \node[smallbox, right of=fulltext, xshift=6cm] (excluded2) {Full-text articles excluded\\(n = 33)\\
        \small Insufficient outcome data (n=15)\\Conference abstracts only (n=11)\\Wrong focus (n=7)};
        
        % Included for analysis
        \node[box, below of=fulltext] (included) {\textbf{Included}\\Studies meeting all inclusion criteria\\(n = 43)\\
        \small Registry analyses (n=12), Cohort studies (n=18),\\Qualitative studies (n=8), RCTs (n=2), Economic evaluations (n=3)};
        
        % Final synthesis
        \node[box, below of=included, fill=gray!20] (synthesis) {\textbf{Critical Appraisal}\\Studies selected for detailed critical appraisal\\(n = 15)\\
