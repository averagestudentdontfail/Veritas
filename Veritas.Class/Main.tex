...
\emph{Proof.} Each conjugate pair \(\omega_j, \overline{\omega_j}\)
(corresponding to some \(\theta_k \in (0, \pi)\)) contributes the real
quadratic
...
\(n\), \(n/2\) quadratics contribute degree \(n\). ∎
...
is unchanged. On \(\mathbb{R}\), note that
\(\overline{x - \omega} = x - \bar{\omega}\). On any domain where the
chosen branch of \(\log\) is compatible with complex conjugation,
\(\log(x - \bar{\omega}) = \overline{\log(x - \omega)}\). Hence
\(\omega\log(x - \omega) + \bar{\omega}\log(x - \bar{\omega})\) is real,
so pairing conjugates yields a real-valued antiderivative; branch
...
\subsection{\texorpdfstring{6.1. The family
\(x^n + a^n\)}{6.1. The family x\^{}n + a\^{}n}}\label{the-family-xn-an}}
...
\subsection{\texorpdfstring{6.2. The family
\(x^n - 1\)}{6.2. The family x\^{}n - 1}}\label{the-family-xn---1}}
...
\subsection{6.3. General rational
