---
title: "Multidisciplinary Management of Acute Burn Injury"
subtitle: "Synthesis of Australasian Evidence-Based Practice"
author: "Kiran Nath"
date: "2025"
lang: en
...
\begin{landscapefigure}[p]  % [p] forces it to its own page
    ...
    \resizebox{0.7\linewidth}{!}{%  % Scale to 70% of line width
        \begin{tikzpicture}[
            ...
            \node[box] (identification) {**Identification**\\Records identified through database searching\\(n = 287)\\
            \small PubMed (n=89), CINAHL (n=67), Cochrane (n=12), EMBASE (n=94), BRANZ (n=25)};
            
            % After duplicates removed
            \node[box, below=1.5cm of identification] (afterdup) {**After Duplicates Removed**\\Records after duplicates removed\\(n = 184)\\
            \small 103 duplicates identified and removed};
            
            % Screening phase
            \node[box, below=1.5cm of afterdup] (screened) {**Screening**\\Records screened by title and abstract\\(n = 184)};
            ...
            \node[box, below=1.5cm of screened] (fulltext) {**Eligibility**\\Full-text articles assessed for eligibility\\(n = 76)};
            ...
            \node[box, below=1.5cm of fulltext] (included) {**Included**\\Studies meeting all inclusion criteria\\(n = 43)\\
            \small Registry analyses (n=12), Cohort studies (n=18),\\Qualitative studies (n=8), RCTs (n=2), Economic evaluations (n=3)};
            
            % Final synthesis
            \node[box, below=1.5cm of included, fill=gray!20] (synthesis) {**Critical Appraisal**\\Studies selected for detailed critical appraisal\\(n = 15)\\
...
\begin{longtable}{@{}p{2.5cm}p{2cm}p{2.5cm}p{1.5cm}p{7cm}@{}}  % @{} removes extra margins for even spacing
    ...
    % First header
    \toprule
    **Study** & **Design** & **Setting** & **Sample** & **Key Findings** \\
    \midrule
    \endfirsthead
...
\begin{enumerate}
    \item **Universal implementation validates effectiveness**: All 17 specialist \glspl{burnunit} across Australia and New Zealand have adopted \gls{mdt} models, with no services maintaining traditional single-discipline approaches. This complete paradigm shift represents powerful implicit evidence for \gls{mdt} superiority.
    
    \item **Process indicators demonstrate coordination quality**: 97\% of patients receive allied health assessment within 48 hours , compared to historical rates of 60\% receiving any allied health input. Units with comprehensive \gls{mdt} protocols achieve 22\% shorter length of stay .
    
    \item **Functional outcomes show sustained improvement**: Structured \gls{mdt} follow-up maintains benefits at 24 months post-injury . \Gls{rtw} rates reach 78\% with formal \gls{mdt} transition protocols versus 54\% without .
    
    \item **Cultural safety and health equity improves outcomes**: Indigenous children experience 2.8-day shorter admissions when Aboriginal Health Workers integrate as core \gls{mdt} members . Culturally-adapted \gls{mdt} models improve follow-up attendance from 52\% to 84\%.
    
    \item **Technology successfully extends \gls{mdt** reach}: Telerehabilitation proves non-inferior to in-person \gls{mdt} therapy while improving rural access . Remote communities can access coordinated care through structured \gls{telehealth} programs .
